/**
 * optim/sgd-scalar.ts  —  REFERENCE ONLY, superseded by optim/sgd.ts
 * ══════════════════════════════════════════════════════════
 * Stochastic Gradient Descent (plain and with momentum), on SCALAR `Value`.
 *
 ...
 * ──────────────────────────────────────────────────────────────────────────
 * This is the version built in Chapter 09, and it is correct for the course
 * as it stood there: Ch 09 comes BEFORE Ch 10, so `TensorValue` did not
 * exist yet and a parameter was a single number. Its update is a plain
 * subtraction of two numbers:
 *
 *     param.data -= learningRate * param.grad;
 *
 * Chapter 14 rebuilds the same two optimizers on `TensorValue`, because
 * Ch 13's `Linear.parameters()` hands back tensors and every real layer from
 * here on has tensor parameters. That version lives in optim/sgd.ts and is
 * what the library exports.
 *
 * Nothing imports this file except its own tests, which still pass — it is
 * kept so you can read the scalar implementation side by side with the
 * tensor one and see exactly what changed. The ARITHMETIC is identical; only
 * the type of `.data` and `.grad` moved from `number` to `Tensor`, so
 * `a - b` became `sub(a, b)` and `lr * g` became `mulScalar(g, lr)`.
 *
 * The classes are suffixed `Scalar` so both files can be exported without a
 * name collision. In Ch 09's doc they are simply `SGD` and `SGDMomentum`.
 *
 * ──────────────────────────────────────────────────────────────────────────
 ...
 */
export class SGDMomentumScalar {
  readonly params: Value[];
  readonly learningRate: number;
