...
# What if a token were a whole word?
...
# What if common pieces became tokens?
...
# Now give the tokenizer more interesting text
...
Now perform the same game: count pairs → choose the most frequent pair → merge it everywhere → count again → repeat. The animation below runs exactly that loop on this corpus.
...
*Figure 1: one merge per frame. The token just created is outlined in red, and the corpus gets shorter every round.*
...
# Something important happened after every merge
...
If we try the second rule first, there is no `lo` yet. The diagram below shows the same thing for two words at once.
...
*Figure 2: read it from the bottom up — every red box sits on top of the pieces an earlier merge produced.*
...
# Now we have a tokenizer. Can it handle a new word?
...
The left column below is that same walk, with every learned rule tried in turn. The right column applies the identical rules in reverse order — which is what the merge-order section warned about.
...
*Figure 3: the same eight rules, in training order and reversed. Same word, four tokens instead of two.*

---

# What does the tokenizer do with spaces?
...
# What if a character was never seen?
...
# Put BPE into the language-model pipeline
