...
| 09 | [ch-09-one-rule-many-layers.md](ch-09-one-rule-many-layers.md) | **Does the one-parameter rule really train a network?**: gradient descent run on Ch 08's own graph, `f = b·sin(a) + b²` — `a`'s gradient arrives through a chain (`b·cos(a)`), `b`'s as a sum over two paths (`sin(a) + 2b`), and the identical update line moves both. Includes a real saddle-point stall at `(0,0)` where 25 iterations barely move, the 08b starting point where `a`'s gradient is exactly zero and the parameter never learns, and what the repeated chain factor predicts about vanishing gradients |
